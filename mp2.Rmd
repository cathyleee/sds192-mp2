...
We are interested to know whether there is any relationship between the advocating donations a candidate received and the number of votes they got in the house elections in 2012. It makes sense to hypothesize that advocating money may increase the chance of getting more votes as money can be used for public relations purposes to boost the popularity of the candidate. Therefore, we join `contributions` and `candidates` tables, and then join the resulting table with `house_elections`. We filter just the "24E" type of transaction, which is the money donated in favor of the candidate. 
```{r}
...
### Make data graphics about the relationship between amount of advocating donations received and number of votes received

We write a function to draw a data graphic for each kind of vote: general votes, primary votes, and runoff votes. 
```{r}
...
*Surprisingly*, despite some fluctuations, all the graphs show similar downward trends. The more money candidates received, the fewer votes they got, in all kinds of votes, across all parties, contrary to popular belief. 

Moreover, we want to know how many votes each party got, how much money it received, and how many wins it had in total as well to compare them against each other and see if the three bar graphs show similar patterns.
```{r}
...
We draw bar graphs in a descending order of y-axis. We can see that Republican and Democratic are the two parties that account for most votes, most money, and most wins, overwhelming more than any other party. Republican outdid Democratic in all three aspects in house elections in 2012. The gap in amount of money seems to be much wider than the gaps in numbers of votes and wins.  